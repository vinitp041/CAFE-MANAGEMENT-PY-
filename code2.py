...
class BakeryOrderManagement:
    ...
    for i in range(10):
        bg_image = Image.open("/Users/vinitpatil/Desktop/CAFE MANAGEMENT PY/bakery.png").convert("RGB").filter(ImageFilter.BLUR)
        self.bg_images.append(ImageTk.PhotoImage(bg_image))

    # Update the background image
    self.root.configure(background="black")
    bg_label = ttk.Label(self.root, image=self.bg_images[0])
    ...
    # Rest of the code...
        ...
        # Call the function to update the background image periodically
    ...
    def update_background(self, idx):
        # Update the background image
        self.root.configure(background="black")
        bg_label = ttk.Label(self.root, image=self.bg_images[idx])
        bg_label.place(relwidth=1, relheight=1)
        # Schedule the next update after 100 milliseconds
        self.root.after(100, self.update_background, (idx + 1) % len(self.bg_images))
